...
- 2026-03-11: Removed the dead `site.email` contact path from `index.md` and `_config.yml`; homepage hero contact should only expose working options; right-nav labels live in `_data/navigation.yml`, with `Startups` removed from the sidebar and `All Posts` renamed to `Blog`; `interests.md` should use `##` section headings to match the site’s section hierarchy.
